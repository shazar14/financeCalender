...
class DataTablesServer:
	...
	query = """SELECT SQL_CALC_FOUND_ROWS %(columns)s FROM %(table)s %(where)s %(order)s %(limit)s""" % dict(columns=', '.join(_columns), table=_sTable, where=self.filtering(), order=self.ordering(), limit=self.paging())
	fh.write(query + '\n')
    ...
    def paging( self ):
        limit = ""
	test = "%d" % self.cgi['iDisplayLength'].value
        #if ( self.cgi['iDisplayStart'] != "" ) and ( self.cgi['iDisplayLength'] != -1 ):
        if ( self.cgi['iDisplayStart'] != "" ) and ( test != -1 ):
